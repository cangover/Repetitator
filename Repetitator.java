...
public class Repetitator
{
	...
	{
		Repetitator part = new Repetitator();
		boolean done = false;
		while(!done){
			boolean good = false;
			while(!good){
	...
	public void run1 ()
	{
		...
		try{
			...
			System.out.println("Error Â­Â­ " + e.toString());
		}
	...
	public void run2 ()
	{
		...
		long minLength = reader.nextLong() - 1;
		BufferedReader buff = openReader(file);
	...
	private String[][] countInstances(BufferedReader buff, boolean isSentence, long num, long minLgth){
		...
		try{
			...
			while(line != null){
				...
				line = line.substring(findNextWord(line, 0));
				String text = "";
				done = false;
				do{
					text = "";
